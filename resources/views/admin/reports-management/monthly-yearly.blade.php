    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Monthly/Yearly Report') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('View aggregated monthly and yearly statistics') }}</p>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
                <form method="GET" action="{{ route('admin.reports-management.monthly-yearly') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <!-- Period Type -->
                    <div>
                        <label for="period" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Period Type') }}</label>
                        <select id="period" name="period" 
                            ...
                            <option value="month" {{ request('period', 'month') == 'month' ? 'selected' : '' }}>{{ __('Monthly') }}</option>
                            <option value="year" {{ request('period') == 'year' ? 'selected' : '' }}>{{ __('Yearly') }}</option>
                        </select>
                    </div>
                    ...
                    <!-- End Date -->
                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('End Date') }}</label>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Donations by Period') }}</h2>
                    <canvas id="donationsChart" height="200"></canvas>
                </div>

                <!-- Requests Chart -->
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Requests by Period') }}</h2>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                                        {{ __('No data found for the selected period') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
