    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Blood Shortage Report') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('View blood types with low stock levels') }}</p>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
                <form method="GET" action="{{ route('admin.reports-management.shortage') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <!-- Threshold -->
                    <div>
                        <label for="threshold" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Low Stock Threshold') }}</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                    <p class="text-sm font-medium text-yellow-800 dark:text-yellow-400">{{ __('Total Shortages') }}</p>
                    <p class="text-2xl font-bold text-yellow-900 dark:text-yellow-300">{{ $total_shortages }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Blood Types with Shortage') }}</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ collect($shortages)->pluck('blood_type')->unique()->count() }} {{ __('types') }}
                    </p>
                </div>
            </div>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                                            {{ $shortage['shortage'] }} {{ __('bags needed') }}
                                        </span>
                                    </td>
                                </tr>
                            @empty
