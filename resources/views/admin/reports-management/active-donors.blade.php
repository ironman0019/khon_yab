    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Active Donors Report') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('View all active donors with ability to donate') }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Total Active Donors') }}</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($total_count) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('By Blood Type') }}</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ collect($by_blood_type)->count() }} {{ __('types') }}
                    </p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('By Province') }}</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ collect($by_province)->count() }} {{ __('provinces') }}
                    </p>
                </div>
            </div>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($donors as $donor)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">
                                            {{ $donor->last_donation_date ? $donor->last_donation_date->format('Y-m-d') : __('Never') }}
                                        </div>
                                    </td>
                                </tr>
                            @empty
