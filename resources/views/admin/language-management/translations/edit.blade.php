<x-admin-layout>
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Page Header -->
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit Translation') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Update translation key and value') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('admin.language-management.translations.update', [$language ?? $translation->language, $translation]) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            >
                                ...
                                @foreach($groups as $groupItem)
                                    <option value="{{ $groupItem }}" {{ old('group', $translation->group) == $groupItem ? 'selected' : '' }}>
                                        {{ ucfirst($groupItem) }}
                                    </option>
                                @endforeach
                            </x-select>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('admin.language-management.translations.index', $language ?? $translation->language) }}" 
