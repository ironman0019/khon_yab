    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Create Hospital User') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Add a new hospital user to the system') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('admin.hospital-user-management.store') }}"
                      x-data="{ 
                          cities: [],
                          init() {
                              @if(old('province_id'))
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-select 
                                ...
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                            >
                                <option value="0" {{ old('status', 0) == 0 ? 'selected' : '' }}>{{ __('admin.Pending') }}</option>
                                <option value="1" {{ old('status') == 1 ? 'selected' : '' }}>{{ __('admin.Active') }}</option>
                                <option value="2" {{ old('status') == 2 ? 'selected' : '' }}>{{ __('admin.Inactive') }}</option>
                                <option value="3" {{ old('status') == 3 ? 'selected' : '' }}>{{ __('admin.Verified') }}</option>
                            </x-select>
