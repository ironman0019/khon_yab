    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Edit Hospital User') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Update hospital user information') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('admin.hospital-user-management.update', $hospitalUser) }}"
                      x-data="{ 
                          cities: [],
                          init() {
                              @if(old('province_id', $hospitalUser->province_id))
                                  fetch('/api/cities?province_id={{ old('province_id', $hospitalUser->province_id) }}')
                                      ...
                                      .then(data => {
                                          cities = data;
                                          @if(old('city_id', $hospitalUser->city_id))
                                              document.getElementById('city_id').value = {{ old('city_id', $hospitalUser->city_id) }};
                                          @endif
                                      })
                                      .catch(() => {});
                              @endif
                    ...
                    <div class="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input 
                                ...
                                :value="old('full_name', $hospitalUser->user->full_name)" 
                                required 
                                autofocus 
                            />
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input 
                                ...
                                type="email" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('email', $hospitalUser->user->email)" 
                                required 
                            />
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('admin.Hospital Information') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Hospital Name -->
                        <div>
                            <x-input-label for="hospital_name" :value="__('admin.Hospital Name')" />
                            <x-text-input 
                                id="hospital_name" 
                                name="hospital_name" 
                                type="text" 
                        ...
                        <!-- Hospital Code -->
                        <div>
                            <x-input-label for="hospital_code" :value="__('admin.Hospital Code')" />
                            <x-text-input 
                                id="hospital_code" 
                                name="hospital_code" 
                                type="text" 
                        ...
                        <div>
                            ...
                            <x-select 
                                ...
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                            >
                                <option value="0" {{ old('status', $hospitalUser->status) == 0 ? 'selected' : '' }}>{{ __('admin.Pending') }}</option>
                                <option value="1" {{ old('status', $hospitalUser->status) == 1 ? 'selected' : '' }}>{{ __('admin.Active') }}</option>
                                <option value="2" {{ old('status', $hospitalUser->status) == 2 ? 'selected' : '' }}>{{ __('admin.Inactive') }}</option>
                                <option value="3" {{ old('status', $hospitalUser->status) == 3 ? 'selected' : '' }}>{{ __('admin.Verified') }}</option>
                            </x-select>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea 
                            ...
                            required
                        >{{ old('address', $hospitalUser->address) }}</textarea>
