    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Edit Receiver') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Update receiver information') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('admin.receiver-management.update', $receiver) }}"
                      x-data="{ 
                          cities: [],
                          init() {
                              @if(old('province_id', $receiver->province_id))
                                  fetch('/api/cities?province_id={{ old('province_id', $receiver->province_id) }}')
                                      ...
                                      .then(data => {
                                          cities = data;
                                          @if(old('city_id', $receiver->city_id))
                                              document.getElementById('city_id').value = {{ old('city_id', $receiver->city_id) }};
                                          @endif
                                      })
                                      .catch(() => {});
                              @endif
                    ...
                    <div class="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input 
                                ...
                                :value="old('full_name', $receiver->user->full_name)" 
                                required 
                                autofocus 
                            />
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input 
                                ...
                                type="email" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('email', $receiver->user->email)" 
                                required 
                            />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="male" {{ old('gender', $receiver->gender) == 'male' ? 'selected' : '' }}>{{ __('admin.Male') }}</option>
                                <option value="female" {{ old('gender', $receiver->gender) == 'female' ? 'selected' : '' }}>{{ __('admin.Female') }}</option>
                                <option value="other" {{ old('gender', $receiver->gender) == 'other' ? 'selected' : '' }}>{{ __('admin.Other') }}</option>
                            </x-select>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="positive" {{ old('rh_factor', $receiver->rh_factor) == 'positive' ? 'selected' : '' }}>{{ __('admin.Positive') }}</option>
                                <option value="negative" {{ old('rh_factor', $receiver->rh_factor) == 'negative' ? 'selected' : '' }}>{{ __('admin.Negative') }}</option>
                            </x-select>
