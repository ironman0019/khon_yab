    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Create User') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Add a new user to the system') }}</p>
            </div>
                ...
                <form method="POST" action="{{ route('admin.user-management.store') }}">
                    ...
                    <div class="mb-6">
                        ...
                        >
                            ...
                            <option value="0" {{ old('user_type') == '0' ? 'selected' : '' }}>{{ __('admin.User') }}</option>
                            <option value="1" {{ old('user_type') == '1' ? 'selected' : '' }}>{{ __('admin.Donor') }}</option>
                            <option value="2" {{ old('user_type') == '2' ? 'selected' : '' }}>{{ __('admin.Hospital User') }}</option>
                        </x-select>
