        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex {{ $isRtl ? 'flex-row-reverse' : '' }} justify-between items-center">
                <div class="{{ $isRtl ? 'text-right' : 'text-left' }}">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Laboratory Management') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Manage all laboratories') }}</p>
                </div>
                <a href="{{ route('admin.laboratory-management.create') }}" 
                   class="inline-flex items-center {{ $isRtl ? 'flex-row-reverse' : '' }} px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors">
                    ...
                    {{ __('admin.Create Laboratory') }}
                </a>
            </div>

            <!-- Filters and Search -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
                <form method="GET" action="{{ route('admin.laboratory-management.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <!-- Search -->
                    <div class="lg:col-span-2">
                        ...
                        <x-text-input 
                            ...
                            placeholder="{{ __('admin.Search by name, email, laboratory name, laboratory code...') }}"
                        />
                    </div>
                            ...
                            @forelse($laboratories as $laboratory)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500 dark:text-gray-400">
                                            {{ $laboratory->contact_person_name }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap {{ $isRtl ? 'text-left' : 'text-right' }} text-sm font-medium">
                                        <div class="flex {{ $isRtl ? 'justify-start' : 'justify-end' }} gap-2">
                                            <a href="{{ route('admin.laboratory-management.show', $laboratory) }}" 
                                            ...
                                            <form method="POST" 
                                                  action="{{ route('admin.laboratory-management.toggle-status', $laboratory) }}" 
                                                  class="inline"
                                            ...
                                            <form method="POST" 
                                                  ...
                                                  class="inline"
                                                  onsubmit="return confirm('{{ __('admin.Are you sure you want to delete this laboratory?') }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('admin.No laboratories found') }}</h3>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('admin.Get started by creating a new laboratory.') }}</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
