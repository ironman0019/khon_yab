    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Edit City') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Update city information') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('admin.province-management.cities.update', [$province, $city]) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            >
                                ...
                                @foreach($provinces as $provinceItem)
                                    <option value="{{ $provinceItem->id }}" {{ old('province_id', $city->province_id) == $provinceItem->id ? 'selected' : '' }}>
                                        {{ $provinceItem->name }}
                                    </option>
                                @endforeach
                            </x-select>
