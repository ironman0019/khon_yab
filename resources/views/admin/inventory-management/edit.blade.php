    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Edit Blood Inventory Entry') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Update blood inventory entry information') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('admin.inventory-management.update', ['inventory_management' => $bloodInventory]) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input 
                                ...
                                :value="old('bag_id', $bloodInventory->bag_id)" 
                                required 
                                autofocus 
                            />
                        ...
                        <div>
                            ...
                            <x-text-input 
                                ...
                                type="number" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('blood_donation_record_id', $bloodInventory->blood_donation_record_id)" 
                                required 
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="A" {{ old('blood_type', $bloodInventory->blood_type) == 'A' ? 'selected' : '' }}>A</option>
                                <option value="B" {{ old('blood_type', $bloodInventory->blood_type) == 'B' ? 'selected' : '' }}>B</option>
                                <option value="AB" {{ old('blood_type', $bloodInventory->blood_type) == 'AB' ? 'selected' : '' }}>AB</option>
                                <option value="O" {{ old('blood_type', $bloodInventory->blood_type) == 'O' ? 'selected' : '' }}>O</option>
                            </x-select>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="positive" {{ old('rh_factor', $bloodInventory->rh_factor) == 'positive' ? 'selected' : '' }}>{{ __('admin.Positive') }}</option>
                                <option value="negative" {{ old('rh_factor', $bloodInventory->rh_factor) == 'negative' ? 'selected' : '' }}>{{ __('admin.Negative') }}</option>
                            </x-select>
                        ...
                        <div>
                            ...
                            >
                                ...
                                @foreach($provinces as $province)
                                    <option value="{{ $province->id }}" {{ old('province_id', $bloodInventory->province_id) == $province->id ? 'selected' : '' }}>
                                        {{ $province->name }}
                                    </option>
                                @endforeach
                            </x-select>
                        ...
                        <div>
                            ...
                            <x-select 
                                ...
                                required
                            >
                                <option value="0" {{ old('status', $bloodInventory->status) == 0 ? 'selected' : '' }}>{{ __('admin.In Stock') }}</option>
                                <option value="1" {{ old('status', $bloodInventory->status) == 1 ? 'selected' : '' }}>{{ __('admin.Used') }}</option>
                                <option value="2" {{ old('status', $bloodInventory->status) == 2 ? 'selected' : '' }}>{{ __('admin.Expired') }}</option>
                                <option value="3" {{ old('status', $bloodInventory->status) == 3 ? 'selected' : '' }}>{{ __('admin.Discarded') }}</option>
                            </x-select>
                        ...
                        <div>
                            ...
                            <x-text-input 
                                ...
                                type="date" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('entry_date', $bloodInventory->entry_date ? $bloodInventory->entry_date->format('Y-m-d') : '')" 
                                required 
                            />
                        ...
                        <!-- Exit Date -->
                        <div>
                            <x-input-label for="exit_date" :value="__('admin.Exit Date (Optional)')" />
                            <x-text-input 
                                id="exit_date" 
                                name="exit_date" 
                                type="date" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('exit_date', $bloodInventory->exit_date ? $bloodInventory->exit_date->format('Y-m-d') : '')"
                            />
                        ...
                        <div>
                            ...
                            <x-text-input 
                                ...
                                type="date" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('expiration_date', $bloodInventory->expiration_date ? $bloodInventory->expiration_date->format('Y-m-d') : '')" 
                                required 
                            />
                    ...
                    <div class="mt-6">
                        ...
                        >{{ old('notes', $bloodInventory->notes) }}</textarea>
