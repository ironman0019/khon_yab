        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex {{ $isRtl ? 'flex-row-reverse' : '' }} justify-between items-center">
                <div class="{{ $isRtl ? 'text-right' : 'text-left' }}">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.Blood Donation Records') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.Manage all blood donation records') }}</p>
                </div>
                <a href="{{ route('admin.blood-donation-management.create') }}" 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
                <form method="GET" action="{{ route('admin.blood-donation-management.index') }}" class="flex flex-col gap-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-text-input 
                                ...
                                placeholder="{{ __('admin.Search by donor name, email, or record ID...') }}"
                            />
                        </div>

                        <!-- Donor Filter -->
                        <div>
                            <label for="donor_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('admin.Donor') }}</label>
                            <x-select 
                                id="donor_id"
                                name="donor_id" 
                                class="block w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="0" @selected(request('status') == '0')>{{ __('admin.Test Pending') }}</option>
                                <option value="1" @selected(request('status') == '1')>{{ __('admin.Safe') }}</option>
                                <option value="2" @selected(request('status') == '2')>{{ __('admin.Unsafe') }}</option>
                                <option value="3" @selected(request('status') == '3')>{{ __('admin.Discarded') }}</option>
                            </x-select>
                        </div>

                        <!-- Donation Type Filter -->
                        <div>
                            <label for="donation_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('admin.Donation Type') }}</label>
                            <x-select 
                                id="donation_type"
                                name="donation_type" 
                                class="block w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                            >
                                ...
                                <option value="0" @selected(request('donation_type') == '0')>{{ __('admin.Whole Blood') }}</option>
                                <option value="1" @selected(request('donation_type') == '1')>{{ __('admin.Plasma') }}</option>
                                <option value="2" @selected(request('donation_type') == '2')>{{ __('admin.Platelets') }}</option>
                            </x-select>
                        </div>
                        ...
                        <!-- Donation Date From -->
                        <div>
                            <label for="donation_date_from" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('admin.From Date') }}</label>
                            <x-text-input 
                        ...
                        <!-- Donation Date To -->
                        <div>
                            <label for="donation_date_to" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('admin.To Date') }}</label>
                            <x-text-input 
                                id="donation_date_to"
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                            {{ $donationTypes[$record->donation_type] ?? __('admin.Unknown') }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap {{ $isRtl ? 'text-left' : 'text-right' }} text-sm font-medium">
                                        <div class="flex {{ $isRtl ? 'justify-start' : 'justify-end' }} gap-2">
                                            <a href="{{ route('admin.blood-donation-management.show', $record) }}" 
                                            ...
                                            <form method="POST" 
                                                  action="{{ route('admin.blood-donation-management.destroy', $record) }}" 
                                                  class="inline"
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-12 text-center">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('admin.No donation records found') }}</h3>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('admin.Get started by creating a new donation record.') }}</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
