        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex {{ $isRtl ? 'flex-row-reverse' : '' }} justify-between items-center">
                <div class="{{ $isRtl ? 'text-right' : 'text-left' }}">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('laboratory.Donation Record Details') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('laboratory.Complete report for donation record #:id', ['id' => $donationRecord->id]) }}</p>
                </div>
                    ...
                    @if($donationRecord->status == 0 && !$donationRecord->bloodTest)
                        <a href="{{ route('laboratory.donation-records.test.create', $donationRecord) }}" 
                           class="inline-flex items-center {{ $isRtl ? 'flex-row-reverse' : '' }} px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors">
                            ...
                            {{ __('laboratory.Add Test Results') }}
                        </a>
                    @endif
                    ...
                    @if($donationRecord->bloodTest && $donationRecord->status != 3)
                        <a href="{{ route('laboratory.donation-records.test.edit', $donationRecord) }}" 
                           class="inline-flex items-center {{ $isRtl ? 'flex-row-reverse' : '' }} px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors">
                            ...
                            {{ __('laboratory.Edit Test Results') }}
                        </a>
                    @endif
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
                <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Donation Record Information') }}</h2>
                </div>
                ...
                <div class="px-6 py-6">
                    <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <!-- Record ID -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Record ID') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">#{{ $donationRecord->id }}</dd>
                        </div>

                        <!-- Donation Type -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Donation Type') }}</dt>
                            <dd class="mt-1 {{ $isRtl ? 'text-right' : 'text-left' }}">
                                ...
                                <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                    {{ $donationTypes[$donationRecord->donation_type] ?? __('laboratory.Unknown') }}
                                </span>
                            </dd>
                        </div>
                        ...
                        <!-- Amount -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Amount') }}</dt>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 {{ $isRtl ? 'text-right' : 'text-left' }}">
                                ...
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $statusColors[$donationRecord->status] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400' }}">
                                    {{ $statusLabels[$donationRecord->status] ?? __('laboratory.Unknown') }}
                                </span>
                            </dd>
                        </div>
                        ...
                        <!-- Donation Date -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Donation Date') }}</dt>
                        ...
                        <!-- Expiration Date -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Expiration Date') }}</dt>
                        ...
                        <!-- Recorded By -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Recorded By') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">
                                {{ $donationRecord->recordedByAdmin->full_name ?? __('laboratory.Unknown') }}
                            </dd>
                        </div>

                        <!-- Submitted By Donor -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Submitted By Donor') }}</dt>
                            <dd class="mt-1 {{ $isRtl ? 'text-right' : 'text-left' }}">
                        ...
                        <!-- Created At -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Created At') }}</dt>
                        ...
                        <!-- Updated At -->
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Updated At') }}</dt>
                    ...
                    @if($donationRecord->notes)
                        <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Notes') }}</dt>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
                <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Donor Information') }}</h2>
                </div>
                <div class="px-6 py-4">
            ...
            @if($donationRecord->province || $donationRecord->city)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Donation Location') }}</h2>
                    </div>
                    <div class="px-6 py-6">
                        <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            @if($donationRecord->bloodInventory->isNotEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Related Blood Inventory') }}</h2>
                    </div>
                    <div class="px-6 py-4">
                        <div class="space-y-4">
                                    ...
                                    <div class="flex {{ $isRtl ? 'flex-row-reverse' : '' }} justify-between items-start">
                                        <div class="{{ $isRtl ? 'text-right' : 'text-left' }}">
                                            ...
                                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                                {{ __('laboratory.Blood Type') }}: {{ $inventory->blood_type }}{{ $inventory->rh_factor == 'positive' ? '+' : '-' }}
                                            </p>
                                        </div>
                                        <div class="{{ $isRtl ? 'text-left' : 'text-right' }}">
                                            @php
                                                $inventoryStatusLabels = [
                                                    0 => __('laboratory.In Stock'),
                                                    1 => __('laboratory.Used'),
                                                    2 => __('laboratory.Expired'),
                                                    3 => __('laboratory.Discarded'),
                                                ];
                                            ...
                                            <span class="px-2 py-1 text-xs font-medium rounded-full {{ $inventoryStatusColors[$inventory->status] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400' }}">
                                                {{ $inventoryStatusLabels[$inventory->status] ?? __('laboratory.Unknown') }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
            ...
            @if($donationRecord->bloodTest)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Blood Test Results') }}</h2>
                    </div>
                    <div class="px-6 py-6">
                        <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Test Date') }}</dt>
                            ...
                            @if($testedBy)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Tested By') }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white {{ $isRtl ? 'text-right' : 'text-left' }}">{{ $testedBy->full_name }}</dd>
                                </div>
                            @endif
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Overall Result') }}</dt>
                                <dd class="mt-1 {{ $isRtl ? 'text-right' : 'text-left' }}">
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                            <h3 class="text-md font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Individual Test Results') }}</h3>
                                ...
                                @foreach($testResults as $testName => $result)
                                    <div class="flex {{ $isRtl ? 'flex-row-reverse' : '' }} justify-between items-center p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                                        ...
                                        <span class="px-2 py-1 text-xs font-medium rounded-full {{ $result ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' }}">
                                            {{ $result ? __('laboratory.Positive') : __('laboratory.Negative') }}
                                        </span>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        @if($donationRecord->bloodTest->test_logs)
                            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Test Logs') }}</dt>
            ...
            @elseif($donationRecord->status == 0)
                ...
                <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800 p-4 mb-6">
                    <p class="text-sm text-yellow-800 dark:text-yellow-300 {{ $isRtl ? 'text-right' : 'text-left' }}">
                        <strong>{{ __('laboratory.Test Pending') }}:</strong> {{ __('laboratory.Blood test results have not been recorded yet. Please add test results.') }}
                    </p>
                </div>
            @endif
        </div>
