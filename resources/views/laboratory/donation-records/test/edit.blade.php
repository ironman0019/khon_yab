    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('laboratory.Edit Blood Test Results') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('laboratory.Update test results for donation record #:id', ['id' => $donationRecord->id]) }}</p>
            </div>
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800 p-4 mb-6">
                <p class="text-sm text-blue-900 dark:text-blue-300 {{ $isRtl ? 'text-right' : 'text-left' }}">
                    <strong>{{ __('laboratory.Donor') }}:</strong> {{ $donationRecord->donor->user->full_name }} ({{ $donationRecord->donor->blood_type }}{{ $donationRecord->donor->rh_factor == 'positive' ? '+' : '-' }}) |
                    <strong>{{ __('laboratory.Donation Date') }}:</strong> {{ $donationRecord->donation_date->format('Y-m-d') }} |
                    <strong>{{ __('laboratory.Amount') }}:</strong> {{ $donationRecord->amount_ml }} ml
                </p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('laboratory.donation-records.test.update', $donationRecord) }}">
                    @csrf
                    @method('PUT')

                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Test Results') }}</h2>
                    <div class="space-y-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div class="mt-2 flex {{ $isRtl ? 'flex-row-reverse' : '' }} gap-6">
                                <label class="inline-flex items-center">
                                    <input 
                                        ...
                                        required
                                    >
                                    <span class="{{ $isRtl ? 'me-2' : 'ms-2' }} text-sm text-gray-700 dark:text-gray-300">{{ __('laboratory.Negative') }}</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input 
                                        type="radio" 
                                        ...
                                        class="rounded border-red-300 dark:border-gray-700 text-red-600 shadow-sm focus:ring-red-500 dark:bg-gray-700 dark:focus:ring-red-600"
                                    >
                                    <span class="{{ $isRtl ? 'me-2' : 'ms-2' }} text-sm text-gray-700 dark:text-gray-300">{{ __('laboratory.Positive') }}</span>
                                </label>
                            </div>
                        ...
                        <!-- HBV Test -->
                        <div>
                            <x-input-label for="hbv_result" :value="__('laboratory.HBV Test Result')" />
                            <div class="mt-2 flex {{ $isRtl ? 'flex-row-reverse' : '' }} gap-6">
                                <label class="inline-flex items-center">
                                    <input 
                                        type="radio" 
                        ...
                        <!-- HCV Test -->
                        <div>
                            <x-input-label for="hcv_result" :value="__('laboratory.HCV Test Result')" />
                            <div class="mt-2 flex {{ $isRtl ? 'flex-row-reverse' : '' }} gap-6">
                                <label class="inline-flex items-center">
                                    <input 
                                        type="radio" 
                        ...
                        <!-- Syphilis Test -->
                        <div>
                            <x-input-label for="syphilis_result" :value="__('laboratory.Syphilis Test Result')" />
                            <div class="mt-2 flex {{ $isRtl ? 'flex-row-reverse' : '' }} gap-6">
                                <label class="inline-flex items-center">
                                    <input 
                                        type="radio" 
                        ...
                        <!-- Malaria Test -->
                        <div>
                            <x-input-label for="malaria_result" :value="__('laboratory.Malaria Test Result')" />
                            <div class="mt-2 flex {{ $isRtl ? 'flex-row-reverse' : '' }} gap-6">
                                <label class="inline-flex items-center">
                                    <input 
                                        type="radio" 
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Test Information') }}</h2>
                    <div class="space-y-6 mb-6">
                        <!-- Test Date -->
                        <div>
                            <x-input-label for="test_date" :value="__('laboratory.Test Date')" />
                            <x-text-input 
                                id="test_date" 
                                name="test_date" 
                                type="date" 
                        ...
                        <!-- Test Logs -->
                        <div>
                            <x-input-label for="test_logs" :value="__('laboratory.Test Logs (Optional)')" />
                            <textarea 
                                id="test_logs" 
                                name="test_logs" 
                                rows="4"
                                ...
                                placeholder="{{ __('laboratory.Additional notes about the test results...') }}"
                            >{{ old('test_logs', $bloodTest->test_logs) }}</textarea>
                            <x-input-error :messages="$errors->get('test_logs')" class="mt-2" />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('laboratory.Optional: Add any additional notes or observations about the test results. A summary log will be automatically updated.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-6 p-4 {{ $bloodTest->overall_result ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' }} rounded-lg border">
                        <p class="text-sm {{ $bloodTest->overall_result ? 'text-red-800 dark:text-red-300' : 'text-green-800 dark:text-green-300' }} {{ $isRtl ? 'text-right' : 'text-left' }}">
                            <strong>{{ __('laboratory.Current Overall Result') }}:</strong> 
                            {{ $bloodTest->overall_result ? __('laboratory.Unsafe') : __('laboratory.Safe') }}
                            @php
                                $testedBy = $bloodTest->getRelationValue('tested_by');
                            @endphp
                            @if($testedBy)
                                | <strong>{{ __('laboratory.Tested By') }}:</strong> {{ $testedBy->full_name }}
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                        <p class="text-sm text-yellow-800 dark:text-yellow-300 {{ $isRtl ? 'text-right' : 'text-left' }}">
                            <strong>{{ __('laboratory.Note') }}:</strong> {{ __('laboratory.If any test result is positive, the blood will be automatically marked as unsafe and discarded from inventory. If all tests are negative, the blood will be marked as safe.') }}
                        </p>
                    </div>
                    ...
                    <div class="flex items-center {{ $isRtl ? 'flex-row-reverse justify-start' : 'justify-end' }} gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('laboratory.donation-records.show', $donationRecord) }}" 
                        ...
                        <button type="submit" 
                            ...
                            {{ __('laboratory.Update Test Results') }}
                        </button>
                    </div>
                </form>
            </div>
