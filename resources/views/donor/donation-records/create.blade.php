    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('donor.Create Donation Request') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('donor.Submit a new blood donation request') }}</p>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <form method="POST" action="{{ route('donor.donation-records.store') }}"
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('donor.Donation Information') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <!-- Donation Type -->
                        <div>
                            <x-input-label for="donation_type" :value="__('donor.Donation Type')" />
                            <x-select 
                                id="donation_type" 
                                name="donation_type" 
                                ...
                                required
                            >
                                <option value="">{{ __('donor.Select Type') }}</option>
                                <option value="0" {{ old('donation_type') == '0' ? 'selected' : '' }}>{{ __('donor.Whole Blood') }}</option>
                                <option value="1" {{ old('donation_type') == '1' ? 'selected' : '' }}>{{ __('donor.Plasma') }}</option>
                                <option value="2" {{ old('donation_type') == '2' ? 'selected' : '' }}>{{ __('donor.Platelets') }}</option>
                            </x-select>
                        ...
                        <!-- Amount (ml) -->
                        <div>
                            <x-input-label for="amount_ml" :value="__('donor.Amount (ml)')" />
                            <x-text-input 
                                id="amount_ml" 
                                name="amount_ml" 
                                type="number" 
                        ...
                        <!-- Donation Date -->
                        <div>
                            <x-input-label for="donation_date" :value="__('donor.Donation Date')" />
                            <x-text-input 
                                id="donation_date" 
                                name="donation_date" 
                                type="date" 
                        ...
                        <div>
                            <x-input-label for="expiration_date" :value="__('donor.Expiration Date')" />
                            <x-text-input 
                                id="expiration_date" 
                            ...
                            />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('donor.Optional - will be calculated automatically if not provided') }}</p>
                            <x-input-error :messages="$errors->get('expiration_date')" class="mt-2" />
                        </div>
                    </div>

                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('donor.Donation Location') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <!-- Province -->
                        <div>
                    ...
                    <!-- Notes -->
                    <div class="mb-6">
                        <x-input-label for="notes" :value="__('donor.Notes')" />
                        <textarea 
                            id="notes" 
                            name="notes" 
                            rows="4"
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('donor.Additional information about the donation') }}</p>
                    </div>
                    ...
                    <div class="flex items-center {{ $isRtl ? 'flex-row-reverse justify-start' : 'justify-end' }} gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button type="submit" 
                            ...
                            {{ __('donor.Submit Donation Request') }}
                        </button>
                    </div>
                </form>
            </div>
