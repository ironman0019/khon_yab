...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('hospital.Create Blood Request') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('hospital.New Blood Request') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('hospital.Fill in the details below to create a blood request') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('hospital.blood-requests.store') }}"
                      x-data="{ 
                          cities: @js($cities),
                          init() {
                              @if(old('province_id'))
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('hospital.Blood Information') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <!-- Blood Type -->
                        <div>
                            <x-input-label for="blood_type" :value="__('hospital.Blood Type')" />
                            <x-select 
                                id="blood_type" 
                                name="blood_type" 
                                ...
                                required
                            >
                                <option value="">{{ __('hospital.Select Blood Type') }}</option>
                        ...
                        <!-- RH Factor -->
                        <div>
                            <x-input-label for="rh_factor" :value="__('hospital.RH Factor')" />
                            <x-select 
                                id="rh_factor" 
                                name="rh_factor" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                required
                            >
                                <option value="">{{ __('hospital.Select RH Factor') }}</option>
                                <option value="positive" {{ old('rh_factor') == 'positive' ? 'selected' : '' }}>{{ __('hospital.Positive') }}</option>
                                <option value="negative" {{ old('rh_factor') == 'negative' ? 'selected' : '' }}>{{ __('hospital.Negative') }}</option>
                            </x-select>
                        ...
                        <!-- Number of Bags -->
                        <div>
                            <x-input-label for="number_of_bags" :value="__('hospital.Number of Bags')" />
                            <x-text-input 
                                id="number_of_bags" 
                                name="number_of_bags" 
                                type="number" 
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 {{ $isRtl ? 'text-right' : 'text-left' }}">{{ __('hospital.Patient Information') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <!-- Patient Name -->
                        <div>
                            <x-input-label for="patient_name" :value="__('hospital.Patient Name')" />
                            <x-text-input 
                                id="patient_name" 
                                name="patient_name" 
                                type="text" 
                        ...
                        <!-- Patient Age -->
                        <div>
                            <x-input-label for="patient_age" :value="__('hospital.Patient Age')" />
                            <x-text-input 
                                id="patient_age" 
                                name="patient_age" 
                                type="number" 
                        ...
                        <div class="md:col-span-2">
                            <x-input-label for="request_reason" :value="__('hospital.Request Reason')" />
                            <x-textarea 
                                id="request_reason" 
                                name="request_reason" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        ...
                        <!-- Contact Number -->
                        <div>
                            <x-input-label for="contact_number" :value="__('hospital.Contact Number')" />
                            <x-text-input 
                                id="contact_number" 
                                name="contact_number" 
                                type="text" 
                        ...
                        <!-- Medical Center -->
                        <div>
                            <x-input-label for="medical_center" :value="__('hospital.Medical Center')" />
                            <x-text-input 
                                id="medical_center" 
                                name="medical_center" 
                                type="text" 
                                class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                :value="old('medical_center', $hospitalUser->hospital_name ?? '')" 
                                required 
                            />
                    ...
                    <!-- Notes -->
                    <div class="mb-6">
                        <x-input-label for="notes" :value="__('hospital.Notes')" />
                        <x-textarea 
                            id="notes" 
                            name="notes" 
                            class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('hospital.Additional notes (optional)') }}</p>
