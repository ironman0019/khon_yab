        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex {{ $isRtl ? 'flex-row-reverse' : '' }} justify-between items-center">
                <div class="{{ $isRtl ? 'text-right' : 'text-left' }}">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('receiver.New Message') }}</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('receiver.Send a message to a user') }}</p>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('receiver.messages.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        >
                            <option value="">{{ __('receiver.Select user type') }}</option>
                            <option value="0" {{ old('recipient_user_type') == '0' ? 'selected' : '' }}>{{ __('receiver.Receiver') }}</option>
                            <option value="1" {{ old('recipient_user_type') == '1' ? 'selected' : '' }}>{{ __('receiver.Donor') }}</option>
                            <option value="2" {{ old('recipient_user_type') == '2' ? 'selected' : '' }}>{{ __('receiver.Laboratory') }}</option>
                            <option value="-1" {{ old('recipient_user_type') == '-1' ? 'selected' : '' }}>{{ __('receiver.Admin') }}</option>
                        </select>
                    ...
                    <div class="mb-4">
                        <label for="subject" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('receiver.Subject') }} <span class="text-gray-400">({{ __('receiver.optional') }})</span>
                        </label>
                        <input 
                            type="text" 
                            id="subject" 
                            ...
                            value="{{ old('subject') }}"
                            class="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-red-500 focus:border-red-500 dark:bg-gray-700 dark:text-white"
                            placeholder="{{ __('receiver.Message subject') }}"
                        >
                        @error('subject')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
