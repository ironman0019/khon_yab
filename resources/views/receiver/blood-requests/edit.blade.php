...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('receiver.Edit Blood Request') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 {{ $isRtl ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('receiver.Edit Blood Request #:id', ['id' => $bloodRequest->id]) }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('receiver.Update blood request details') }}</p>
            </div>

            <!-- Form -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('receiver.blood-requests.update', $bloodRequest) }}"
                      x-data="{ 
                          cities: @js($cities),
                          init() {
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="positive" {{ old('rh_factor', $bloodRequest->rh_factor) == 'positive' ? 'selected' : '' }}>{{ __('receiver.Positive') }}</option>
                                <option value="negative" {{ old('rh_factor', $bloodRequest->rh_factor) == 'negative' ? 'selected' : '' }}>{{ __('receiver.Negative') }}</option>
                            </x-select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-textarea 
                                ...
                                rows="3"
                                required
                            >{{ old('request_reason', $bloodRequest->request_reason) }}</x-textarea>
                    ...
                    <div class="mb-6">
                        ...
                        <x-textarea 
                            ...
                            class="block mt-1 w-full border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                            rows="3"
                        >{{ old('notes', $bloodRequest->notes) }}</x-textarea>
                    ...
                    <div class="flex {{ $isRtl ? 'flex-row-reverse justify-start' : 'justify-end' }} gap-4">
                        <a href="{{ route('receiver.blood-requests.show', $bloodRequest) }}" 
