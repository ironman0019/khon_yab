...
{
    /**
     * Display a listing of the hospital's blood requests.
     */
    ...
    public function create(): View
    {
        ...
        $cities = $hospitalUser->province_id
            ? \App\Models\City::where('province_id', $hospitalUser->province_id)->get()
            : collect();

        return view('hospital.blood-requests.create', compact('provinces', 'cities', 'hospitalUser'));
    }
    ...
    public function store(StoreBloodRequestRequest $request): RedirectResponse
    {
        ...
        return redirect()->route('hospital.blood-requests.index')
            ->with('success', __('hospital.Blood request created successfully.'));
    }
    ...
    public function show(BloodRequest $bloodRequest): View
    {
        ...
        if ($bloodRequest->requested_by !== $user->id) {
            abort(403, __('hospital.Unauthorized access.'));
        }
        ...
        return view('hospital.blood-requests.show', compact('bloodRequest'));
    }
    ...
    public function print(BloodRequest $bloodRequest): View
    {
        ...
        if (! in_array($bloodRequest->status, [BloodRequestStatus::Approved->value, BloodRequestStatus::Completed->value])) {
            return redirect()->route('hospital.blood-requests.show', $bloodRequest)
                ->with('error', __('hospital.Only approved or completed requests can be printed.'));
        }
        ...
        return view('hospital.blood-requests.print', compact('bloodRequest'));
    }
