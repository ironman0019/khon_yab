...
{
    /**
     * Display a listing of the receiver's blood requests.
     */
    public function index(Request $request): View
    {
        ...
        return view('receiver.blood-requests.index', compact('bloodRequests'));
    }
    ...
    public function create(): View
    {
        ...
        $cities = $receiver->province_id
            ? City::where('province_id', $receiver->province_id)->get()
            : collect();

        return view('receiver.blood-requests.create', compact('provinces', 'cities', 'receiver'));
    }
    ...
    public function store(StoreBloodRequestRequest $request): RedirectResponse
    {
        ...
        return redirect()->route('receiver.blood-requests.index')
            ->with('success', __('receiver.Blood request created successfully.'));
    }
    ...
    public function show(BloodRequest $bloodRequest): View
    {
        ...
        if ($bloodRequest->requested_by !== $user->id) {
            abort(403, __('receiver.Unauthorized access.'));
        }
        ...
        return view('receiver.blood-requests.show', compact('bloodRequest'));
    }
    ...
    public function edit(BloodRequest $bloodRequest): View
    {
        ...
        if ($bloodRequest->status !== BloodRequestStatus::Pending->value) {
            return redirect()->route('receiver.blood-requests.show', $bloodRequest)
                ->with('error', __('receiver.Only pending requests can be edited.'));
        }

        $provinces = Province::all();
        $cities = $bloodRequest->province_id
            ? City::where('province_id', $bloodRequest->province_id)->get()
            : collect();

        return view('receiver.blood-requests.edit', compact('bloodRequest', 'provinces', 'cities'));
    }

    /**
     * Update the specified blood request in storage.
     */
    public function update(StoreBloodRequestRequest $request, BloodRequest $bloodRequest): RedirectResponse
    {
        $user = Auth::user();
        ...
        if ($bloodRequest->status !== BloodRequestStatus::Pending->value) {
            return redirect()->route('receiver.blood-requests.show', $bloodRequest)
                ->with('error', __('receiver.Only pending requests can be updated.'));
        }
        ...
        return redirect()->route('receiver.blood-requests.show', $bloodRequest)
            ->with('success', __('receiver.Blood request updated successfully.'));
    }
    ...
    public function destroy(BloodRequest $bloodRequest): RedirectResponse
    {
        ...
        if ($bloodRequest->status !== BloodRequestStatus::Pending->value) {
            return redirect()->route('receiver.blood-requests.show', $bloodRequest)
                ->with('error', __('receiver.Only pending requests can be cancelled.'));
        }

        $bloodRequest->delete();

        return redirect()->route('receiver.blood-requests.index')
            ->with('success', __('receiver.Blood request cancelled successfully.'));
    }
    ...
    public function print(BloodRequest $bloodRequest): View
    {
        ...
        if (! in_array($bloodRequest->status, [BloodRequestStatus::Approved->value, BloodRequestStatus::Completed->value])) {
            return redirect()->route('receiver.blood-requests.show', $bloodRequest)
                ->with('error', __('receiver.Only approved or completed requests can be printed.'));
        }
        ...
        return view('receiver.blood-requests.print', compact('bloodRequest'));
    }
